  ...
  Component({ bridge, theme }: PluginProps) {
    ...
    const loadData = useCallback(async () => {
      const results = await Promise.allSettled([
        api(DEVICE, 'device.calendar.today'),
        api(DEVICE, 'device.calendar.upcoming', { days: 7 }),
        api(MEMORY, 'memory.list'),
        api(DEVICE, 'device.contacts.search', { query: '' }),
      ]);
      const parse = (r: any) => {
      ...
      try { const today = parse(results[0]); if (today) setTodayEvents(today.events || (Array.isArray(today) ? today : [])); } catch {}
      try {
        const upcoming = parse(results[1]);
        if (upcoming) {
          let events = upcoming.events || [];
          if (upcoming.days) { events = []; upcoming.days.forEach((d: any) => (d.events || []).forEach((e: any) => events.push({ ...e, date: d.date }))); }
          setUpcomingEvents(events);
        }
      } catch {}
      try { const mem = parse(results[2]); if (mem) setMemories((mem.memories || []).slice(0, 5)); } catch {}
      try { const cont = parse(results[3]); if (cont) setContacts((cont.contacts || []).slice(0, 5)); } catch {}
      setLoading(false);
    }, [api]);
    ...
    const colors = theme?.colors || {
      ...
      surface: '#111820', border: 'rgba(255,255,255,0.08)',
    };
          ...
          {contacts.length === 0 ? (
            ...
            <View key={i} style={[s.contactRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <View>
                ...
                <Text style={[s.contactRel, { color: colors.textMuted }]}>{c.relationship || ''}{c.phone ? ` \u2022 ${c.phone}` : ''}</Text>
              </View>
            </View>
          ))}
        </Section>
...
function Section({ icon, title, badge, colors, children }: {
  icon: string; title: string; badge?: string; colors: any; children: React.ReactNode;
}) {
  return (
