/**
 * Latvian Progress — RN Plugin (rn-src variant, for mobile bundle)
 * Reads memory-mcp under the `latvian` tag; renders CEFR level, streak, XP,
 * 4 category cards, badges, and weak-spots. Mirrors plugins/latvian-progress
 * but uses react-native + @adas/plugin-sdk imports per the rn-src convention.
 */
...
const BADGE_CATALOG: Array<{ id: string; emoji: string; label: string; cond: (s: Stats) => boolean }> = [
  { id: 'first_word',     emoji: '🌱', label: 'מילה ראשונה',         cond: (s) => s.vocab.total >= 1 },
  { id: 'ten_words',      emoji: '📚', label: '10 מילים',             cond: (s) => s.vocab.total >= 10 },
  { id: 'fifty_words',    emoji: '🎒', label: '50 מילים',             cond: (s) => s.vocab.total >= 50 },
  { id: 'hundred_words',  emoji: '🏅', label: '100 מילים',            cond: (s) => s.vocab.total >= 100 },
  { id: 'first_mastered', emoji: '✨', label: 'מילה בשליטה',          cond: (s) => s.vocab.mastered >= 1 },
  { id: 'first_case',     emoji: '🎓', label: 'יחסה ראשונה',          cond: (s) => s.grammar.casesTouched >= 1 },
  { id: 'all_cases',      emoji: '🏛️', label: 'כל 6 היחסות',          cond: (s) => s.grammar.casesTouched >= 6 },
  { id: 'streak_3',       emoji: '🔥', label: 'רצף 3 ימים',           cond: (s) => s.streak.current >= 3 },
  { id: 'streak_7',       emoji: '🔥', label: 'רצף שבוע',             cond: (s) => s.streak.current >= 7 },
  { id: 'streak_30',      emoji: '🔥', label: 'רצף חודש',             cond: (s) => s.streak.current >= 30 },
  { id: 'level_a2',       emoji: '🚀', label: 'הגעת ל-A2',            cond: (s) => cefrIndex(s.level) >= 2 },
  { id: 'level_b1',       emoji: '🛫', label: 'הגעת ל-B1',            cond: (s) => cefrIndex(s.level) >= 3 },
];
...
function buildStats(memories: any[]): Stats {
  const now = Date.now();
  const buckets: Record<string, any[]> = { vocab: [], grammar: [], mistake: [], state: [] };
...
function LatvianProgressComponent({ bridge, native, theme }: PluginProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      const raw = await api.call('memory.list', { tags: ['latvian'], limit: 1000 }, MEMORY);
  ...
  const startReview = useCallback(() => {
    ...
    try { (bridge as any)?.send?.({ type: 'message', text: 'בוא נתחיל חזרה יומית של מילים' }); } catch {}
  }, [bridge, native]);
  ...
  const colors = theme?.colors || {
    ...
    accent: '#FF7A28', success: '#5A8A5C', error: '#C15545',
  };
  const accent = (colors as any).accent || '#FF7A28';
  ...
  return (
    <ScrollView style={[s.container, { backgroundColor: colors.bg }]} contentContainerStyle={s.containerInner}>
      ...
      <View style={s.statsRow}>
        ...
        <View style={[s.statCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={s.statEmoji}>🎯</Text>
          <Text style={[s.statBig, { color: colors.text }]}>{stats.dailyGoal.done}/{stats.dailyGoal.target}</Text>
          <View style={[s.miniBar, { backgroundColor: colors.border }]}>
            <View style={[s.miniBarFill, { width: `${Math.round(dailyPct * 100)}%`, backgroundColor: dailyPct >= 1 ? ((colors as any).success || '#5A8A5C') : accent }]} />
          </View>
      ...
      <View style={s.gridRow}>
        <CategoryCard colors={colors} accent={accent}
          ...
          breakdown={[
            { label: 'בשליטה', value: stats.vocab.mastered, color: (colors as any).success || '#5A8A5C' },
        ...
        <CategoryCard colors={colors} accent={accent}
          ...
          breakdown={[
            { label: 'בשליטה', value: stats.grammar.solid, color: (colors as any).success || '#5A8A5C' },
            { label: 'רעוע',   value: stats.grammar.shaky, color: (colors as any).error || '#C15545' },
          ]}
        />
      </View>
      <View style={s.gridRow}>
        ...
        <CategoryCard colors={colors} accent={accent}
          ...
          breakdown={[
            { label: 'תוקנו', value: stats.conversation.correctedTurns, color: (colors as any).error || '#C15545' },
          ]}
        />
      </View>
      ...
      <View style={[s.ctaCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <View style={s.ctaRow}>
          <View style={{ flex: 1 }}>
            <Text style={[s.ctaTitle, { color: colors.text }]}>🔁 לחזרה עכשיו: {stats.vocab.dueNow} מילים</Text>
      ...
      <View style={s.section}>
        ...
        <View style={s.badgesRow}>
          {BADGE_CATALOG.map((b) => {
            ...
            return (
              <View key={b.id} style={[s.badge, { backgroundColor: colors.surface, borderColor: earned ? accent : colors.border, opacity: earned ? 1 : 0.45 }]}>
                <Text style={s.badgeEmoji}>{earned ? b.emoji : '🔒'}</Text>
                <Text style={[s.badgeLabel, { color: earned ? colors.text : colors.textMuted }]} numberOfLines={2}>{b.label}</Text>
              </View>
            );
          })}
        </View>
        ...
        <View style={s.section}>
          ...
          {stats.mistakes.map((m, i) => (
            <View key={i} style={[s.weakRow, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Text style={[s.weakKind, { color: (colors as any).error || '#C15545' }]}>{m.kind}</Text>
      ...
      {loadError && (
        <Text style={[s.errorFoot, { color: (colors as any).error || '#C15545' }]}>
